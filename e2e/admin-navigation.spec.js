...
test('mobile login and logout remain usable', async ({ page }) => {
 ...
 await navigate(page,'Overview'); await expect(page.locator('#countries')).toContainText('No country data',{timeout:10000});
 await expect(page.locator('#trafficChart .chart-target')).toHaveCount(30);
 for (const value of await page.locator('#trafficChart .chart-target').evaluateAll(nodes => nodes.map(node => node.getAttribute('aria-label')))) expect(value).toMatch(/0 visits$/);
 });
