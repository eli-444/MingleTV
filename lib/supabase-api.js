...
function metrics(data) {
  ...
  return { ...data, generatedAt: new Date().toISOString(), daily, monthly: data.monthly.map(row => ({ ...row, month: row.month.slice(0, 7) })), today: daily.find(row => row.date === new Date().toISOString().slice(0, 10)) || { pageviews: 0, peak: 0, reports: 0 } };
}
